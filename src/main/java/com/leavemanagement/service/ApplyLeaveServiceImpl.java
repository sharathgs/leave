...
public class ApplyLeaveServiceImpl implements ApplyLeaveService {
	...
	public ApplyLeaveResponseDto applyLeave(ApplyLeaveRequestDto applyLeaveRequestDto) {
		...
		if (applyLeaveUtil.getDateInFormat(applyLeaveRequestDto.getFromDate()).compareTo(LocalDate.now()) < 0
				|| (applyLeaveUtil.getDateInFormat(applyLeaveRequestDto.getToDate())
						.compareTo(applyLeaveUtil.getDateInFormat(applyLeaveRequestDto.getFromDate())) < 0)) {

			throw new InvalidDateException(ApplyLeaveUtil.INVALID_DATE_EEXCEPTION);
		}

		Optional<MyLeaves> myLeave = myLeavesRepository
				.findByForDate(applyLeaveUtil.getDateInFormat(applyLeaveRequestDto.getFromDate()));

		if (myLeave.isPresent()) {
			throw new AlreadyApplyLeaveException(ApplyLeaveUtil.ALREADY_APPLY_LEAVE_EEXCEPTION);
		}
		...
		int noOfLeaves = applyLeaveUtil.calculateLeave(applyLeaveRequestDto.getFromDate(), applyLeaveRequestDto.getToDate());
